...
const PersonalPage = () => {
  ...
  const getdata = async () => {
    try {
      ...
      const response = await Axios.get(`${process.env.Server_Path}/PersonalPage/${id}`,
      {
        withCredentials: true,
        credentials: 'include',
        headers: {
          'Access-Control-Allow-Origin': `${process.env.Server_Path}`,
                      ...
                      <tbody className="bg-gray-800">
                        <tr className="bg-black bg-opacity-20">
                          ...
                          const con =  confirm("You have conform delete a event... ");
                           try {
                            if(con === true){
                            await Axios.delete(`${process.env.Server_Path}/PersonalPage/${info.id}`);
