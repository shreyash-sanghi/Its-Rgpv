...
// const form = useRef();
   const RegSubmit = async (event)=>{
    event.preventDefault();
      ...
      try {
        const response = await Axios.post(`http://its-rgpv-backend.vercel.app/registration/${id}`,{
 ...
 const getrequest = async()=>{
  try {
   const resp =  await Axios.get(`http://its-rgpv-backend.vercel.app/registration/${id}`);
  } catch (error) {
    if(error.response.request.status === 401){
      navigate('/errorpage');
  }
